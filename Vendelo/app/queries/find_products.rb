class FindProducts
  ...
  # clase optimizada de peticiones queries
  def initialize(products = initialize_scope)
    @products = products
  end
  def call(params = {})
    # solicitud de parametros de filtrad
    scoped = products
  ...
  def initialize_scope # por default se manda este metodo si no se le pasa un parametro
  ...
  def query_text(scoped, query_text)
